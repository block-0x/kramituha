require "application_system_test_case"

class VideosTest < ApplicationSystemTestCase
  setup do
    @video = videos(:one)
  end

  test "visiting the index" do
    visit videos_url
    assert_selector "h1", text: "Videos"
  end

  test "creating a Video" do
    visit videos_url
    click_on "New Video"

    fill_in "Channel url", with: @video.channel_url
    fill_in "Create stamp", with: @video.create_stamp
    fill_in "Description", with: @video.description
    fill_in "Dislike", with: @video.dislike
    fill_in "Like", with: @video.like
    fill_in "Order", with: @video.order
    fill_in "Queriy", with: @video.queriy
    fill_in "Tag", with: @video.tag
    fill_in "Title", with: @video.title
    fill_in "Video length", with: @video.video_length
    fill_in "Video url", with: @video.video_url
    fill_in "View", with: @video.view
    click_on "Create Video"

    assert_text "Video was successfully created"
    click_on "Back"
  end

  test "updating a Video" do
    visit videos_url
    click_on "Edit", match: :first

    fill_in "Channel url", with: @video.channel_url
    fill_in "Create stamp", with: @video.create_stamp
    fill_in "Description", with: @video.description
    fill_in "Dislike", with: @video.dislike
    fill_in "Like", with: @video.like
    fill_in "Order", with: @video.order
    fill_in "Queriy", with: @video.queriy
    fill_in "Tag", with: @video.tag
    fill_in "Title", with: @video.title
    fill_in "Video length", with: @video.video_length
    fill_in "Video url", with: @video.video_url
    fill_in "View", with: @video.view
    click_on "Update Video"

    assert_text "Video was successfully updated"
    click_on "Back"
  end

  test "destroying a Video" do
    visit videos_url
    page.accept_confirm do
      click_on "Destroy", match: :first
    end

    assert_text "Video was successfully destroyed"
  end
end
